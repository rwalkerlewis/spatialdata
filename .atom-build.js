...
var getBuildDir = function() {
    ...
    if (process.env.SPATIALDATA_BUILDDIR == undefined) {
	atom.notifications.addError('Build directory not set', {
	    detail: "SPATIALDATA_BUILDDIR environment variable not set. Please set SPATIALDATA_BUILDDIR to your spatialdata build directory and then restart Atom."
	});
    } // if
    
    return process.env.SPATIALDATA_BUILDDIR;
} // getBuildDir
...
var builds = function() {
    ...
    addMakeTarget(all, "build-python", ["-C spatialdata"])
    ...
    addMakeTarget(all, "install-python", ["install", "-C spatialdata"])
